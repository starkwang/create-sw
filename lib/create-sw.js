...
function generate(params, callback) {
  return new Promise(function (resolve, reject) {
    ...
    var datas = params.externals.map(function (url) {
      return getUrl(url);
    });
    Promise.all(datas).then(function (arr) {
      return arr.map(function (data, index) {
        return [params.externals[index], getHash(data)];
      });
    }).then(function (externalUrlAndHash) {
